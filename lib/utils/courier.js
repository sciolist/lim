...
module.exports = function createCourier(pending) {
  ...
  Courier.prototype.fill = function fill(input) {
    ...
    for (var i=0; i<prefix.length; ++i) {
      if(prefix[i] === placeholder) { remain += 1; }
      else if(prefix[i] === pending) { remain += 1; nestedRemain += 1; }
      else if(hasCourier(prefix[i])) {
    ...
    for (var i = 0; i < prefix.length; ++i) {
      ...
      if (prefix[i] === pending) nestedRemain += 1; // input shift returned new pending object
      else if (!getPending(i)) remain -= 1; // no pending value required for this position
    }
    return new Courier(this.wrapped, {
      name: this.name,
      prefix: prefix,
