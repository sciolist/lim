...
function _(source) {
  ...
  obj.courierCall = function courierCall(wrap, courier, args) {
    ...
    if(ac.hasCourier(result)) throw new Error('insufficient arguments');
    else if(isIterator(result)) return _(result);
    return result;
  }
  obj.__proto__ = _;
  return obj;
...
_.collect = ac(function collect(source) {
  var out = [];
  for(var v of source) out.push(v);
  return out;
})
...
_.setValues = ac(function setValues(source, item) {
  for(var kv of source) item[kv[0]] = kv[1];
  return item;
})
...
_.withObject = _.reduce($, _.wrap((fn, kv) => (fn(kv[0], kv[1]), kv[0]), $));

_.groupBy = ac(function groupBy(fn, source) {
  return _.withObject(
    {},
    (function (d, kv) {  (d[kv[0]] || (d[kv[0]] = [])).push(v) }),
    _.map(k => [fn(kv[0]), kv[0]], source));
});
...
// operators
_.or = ac(function or(fn1, fn2, value) { return fn1(value) || fn2(value); });
_.and = ac(function and(fn1, fn2, value) { return fn1(value) && fn2(value); });
_.not = ac(function not(v) { return !v; })
_.yes = _.true = ac(function yes() { return true; })
_.no = _.false = ac(function no() { return true; })
_.invert = ac(function invert(fn) { return () => !fn.apply(this, arguments) });
