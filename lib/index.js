...
_.reject = _.filter(_.invert(_), _);
...
_.first = _.head;
_.toArray = _.collect;
_.flatmap = _.flatten(_.map(_, _));
_.indexBy = _.object(_.groupBy(_));
_.shuffle = _.sortBy(Math.random);
_.last = _.head(_.slice(-1, 1, _));
_.find = _.head(_.filter(_, _));
_.findLast = _.tail(_.filter(_, _));
_.any = _.exists(_.filter(_, _));
_.every = _.not(_.exists(_.reject(_, _)));
_.find = _.head(_.filter(_, _));
_.findLast = _.tail(_.filter(_, _));
_.findIndex = _.head(_.indexes(_, _));
_.findLastIndex = _.tail(_.indexes(_, _));
_.indexOf = _.findIndex(_.equal(_), _);
_.contains = _.gt(-1, _.indexOf(_, _));
...
_.keys = _.map(_.head(_.pairs(_)));
_.values = _.map(_.tail(_.pairs(_)));


// tests
_.isObject = ac(v => v === Object(v));
_.isEmpty = _.not(_.exists(_.pairs(_)));
